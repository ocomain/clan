...
function buildAbandonedReminderHtml({ firstName, tierName, ctaUrl }) {
  const safeFirst = firstName || 'friend';
  // CTA: a pre-filled checkout link. The live sender
  // (daily-abandoned-sweep.js) builds this per-recipient as
  // /api/create-checkout?tier=<slug>&email=<email>; for the preview /
  // any caller that doesn't supply one, fall back to a sample link.
  const button = ctaUrl || 'https://www.ocomain.org/api/create-checkout?tier=guardian-ind&email=name@example.com';
  return `<!DOCTYPE html>
...
<div style="max-width:580px;margin:0 auto;background:#F8F4EC">
  ...
  <div style="padding:40px">
    ...
    <div style="text-align:center;margin-bottom:14px">
      <a href="${button}" style="display:inline-block;background:#B8975A;color:#0C1A0C;font-family:sans-serif;font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;text-decoration:none;padding:16px 36px;border-radius:1px">Try your payment again</a>
    </div>
